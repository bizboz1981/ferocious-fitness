...
class ClassType(models.Model):
    """
    Represents a type of class offered, such as a group class or personal
    training session.
...
class Session(models.Model):
    """
    Represents a fitness session, which can be either a group class or a
    personal training session.

    Attributes:
        title (str): The title of the session (e.g., "Yoga", "HIIT",
        "Circuits").
        description (str, optional): A brief description of the session.
        session_type (ClassType): The type of session, linked to the ClassType
        model.
        date (date): The date on which the session is scheduled.
        time (time): The time at which the session is scheduled.
        location (str, optional): The location where the session will take
        place.
        max_participants (int): The maximum number of participants allowed in
        the session. Defaults to 1 for personal training sessions.
        current_participants (int): The current number of participants
        enrolled in the session. Defaults to 0 and is not editable.

    Methods:
        __str__(): Returns a string representation of the session, including
        its title, date, and time.
        spots_remaining(): Returns the number of spots remaining in the
        session.
...
class Booking(models.Model):
    ...
    Attributes:
        ...
        booking_date (DateTimeField): The date and time when the booking was
        made.

    Meta:
        unique_together (tuple): Ensures that a user cannot book the same
        session more than once.

    Methods:
        __str__(): Returns a string representation of the booking.
        save(*args, **kwargs): Saves the booking and increments the session's
        participant count.
        delete(*args, **kwargs): Deletes the booking and decrements the
    ...
    def __str__(self):
        return (
            f"{self.user.username} booked {self.session.title} on "
            f"{self.session.date}"
        )
